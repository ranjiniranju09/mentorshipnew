...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.chapterTest.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.chapter-tests.update", [$chapterTest->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('moduleid') ? 'is-invalid' : '' }}" name="moduleid_id" id="moduleid_id" required>
                    @foreach($moduleids as $id => $entry)
                        <option value="{{ $id }}" {{ (old('moduleid_id') ? old('moduleid_id') : $chapterTest->moduleid->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('chapter') ? 'is-invalid' : '' }}" name="chapter_id" id="chapter_id" required>
                    @foreach($chapters as $id => $entry)
                        <option value="{{ $id }}" {{ (old('chapter_id') ? old('chapter_id') : $chapterTest->chapter->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('test_title') ? 'is-invalid' : '' }}" type="text" name="test_title" id="test_title" value="{{ old('test_title', $chapterTest->test_title) }}" required>
