...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.moduleresourcebank.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.moduleresourcebanks.update", [$moduleresourcebank->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('module') ? 'is-invalid' : '' }}" name="module_id" id="module_id" required>
                    @foreach($modules as $id => $entry)
                        <option value="{{ $id }}" {{ (old('module_id') ? old('module_id') : $moduleresourcebank->module->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('chapterid') ? 'is-invalid' : '' }}" name="chapterid_id" id="chapterid_id" required>
                    @foreach($chapterids as $id => $entry)
                        <option value="{{ $id }}" {{ (old('chapterid_id') ? old('chapterid_id') : $moduleresourcebank->chapterid->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $moduleresourcebank->title) }}" required>
                @if($errors->has('title'))
                    <div class="invalid-feedback">
                        {{ $errors->first('title') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('link') ? 'is-invalid' : '' }}" type="text" name="link" id="link" value="{{ old('link', $moduleresourcebank->link) }}">
                @if($errors->has('link'))
                    <div class="invalid-feedback">
                        {{ $errors->first('link') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('descriptivematerial') ? 'is-invalid' : '' }}" name="descriptivematerial" id="descriptivematerial">{!! old('descriptivematerial', $moduleresourcebank->descriptivematerial) !!}</textarea>
