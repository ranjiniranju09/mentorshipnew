    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.questions.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('mcq') ? 'is-invalid' : '' }}" name="mcq" id="mcq">
                    <option value="Yes" {{ old('mcq', '1') == '1' ? 'selected' : '' }}>Yes</option>
                    <option value="No" {{ old('mcq', '1') == '0' ? 'selected' : '' }}>No</option>
                </select>
                @if($errors->has('mcq'))
                    <div class="invalid-feedback">
                        {{ $errors->first('mcq') }}
                ...
                @endif
                <!-- <span class="help-block">{{ trans('cruds.question.fields.mcq_helper') }}</span> -->
            </div>
...
<script>
    Dropzone.options.questionImageDropzone = {
        ...
        success: function (file, response) {
            ...
            // Add mcq field value to the form (if not already present)
            if (!$('form').find('input[name="mcq"]').length) {
                const mcqValue = $('#mcq').val(); // Get the value of the mcq select field
                $('form').append('<input type="hidden" name="mcq" value="' + mcqValue + '">');
            }
        },
        ...
        init: function () {
            @if(isset($question) && $question->question_image)
                        ...
                        // Add mcq field value for existing questions
                        if (!$('form').find('input[name="mcq"]').length) {
                            const mcqValue = {{ $question->mcq ?? '0' }}; // Use default value if mcq is not set
                            $('form').append('<input type="hidden" name="mcq" value="' + mcqValue + '">');
                        }
            @endif
        },
