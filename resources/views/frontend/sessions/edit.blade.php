                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.sessions.update", [$session->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="modulename_id" id="modulename_id" required>
                                @foreach($modulenames as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('modulename_id') ? old('modulename_id') : $session->modulename->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="mentorname_id" id="mentorname_id" required>
                                @foreach($mentornames as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('mentorname_id') ? old('mentorname_id') : $session->mentorname->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="menteename_id" id="menteename_id" required>
                                @foreach($menteenames as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('menteename_id') ? old('menteename_id') : $session->menteename->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control datetime" type="text" name="sessiondatetime" id="sessiondatetime" value="{{ old('sessiondatetime', $session->sessiondatetime) }}" required>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="sessionlink" id="sessionlink" value="{{ old('sessionlink', $session->sessionlink) }}" required>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="session_title" id="session_title" value="{{ old('session_title', $session->session_title) }}" required>
                        ...
                        <div class="form-group">
                            ...
                            @foreach(App\Session::SESSION_DURATION_MINUTES_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="session_duration_minutes_{{ $key }}" name="session_duration_minutes" value="{{ $key }}" {{ old('session_duration_minutes', $session->session_duration_minutes) === (string) $key ? 'checked' : '' }} required>
                                    <label for="session_duration_minutes_{{ $key }}">{{ $label }}</label>
                                </div>
                            @endforeach
