        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header">
                    {{ trans('global.edit') }} {{ trans('cruds.guestSessionAttendance.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.guest-session-attendances.update", [$guestSessionAttendance->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="session_title_id" id="session_title_id" required>
                                @foreach($session_titles as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('session_title_id') ? old('session_title_id') : $guestSessionAttendance->session_title->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
